...
- [ ] **Get more measured within-family negatives — now known to require literature curation, not more mining.** Still the binding constraint. PAZy is exhausted: of its 155 non-PET enzymes only 26 share a 30% cluster with a PET-active one and only 15 hit the profile library, giving at most 29 in union and just 3–7 shared clusters. The conclusion is robust to which boundary is used (0.493 cluster-defined, 0.279 profile-defined), so the gap is real data, not definition choice. The remaining route is the primary literature: papers assaying several enzymes under one protocol report the inactive ones, which is the same pass as the ordinal-ranking item below
