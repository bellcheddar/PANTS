...
- [ ] **Obtain the HGMP01 sequence (needs institutional access).** Searched exhaustively and it is not programmatically retrievable: zero hits in UniProt (four query forms) and in NCBI protein and nuccore, no sequence records linked from the PMID, no accessions on the PubMed record, ScienceDirect returns 403, and Europe PMC reports `isOpenAccess N`, `inEPMC N`, `hasSuppl N` with no data availability statement. MGnify has no protein-search endpoint either. The sequence is in a supplementary PDF behind a subscription (doi 10.1016/j.ijbiomac.2024.137732), so this needs a human with access rather than more automation. Until it lands, the gut candidates cannot be compared against the one enzyme known to be there
